...
void GameScene::start()
{
	...
	explosionTexture = loadTexture("gfx/explosion1.png");
...
void GameScene::draw()
{
	...
	drawText(610, 20, 255, 255, 255, TEXT_CENTER, "POWER-UPS: %d", player->getPowerUpCount());
...
void GameScene::doSpawnLogic()
{
	...
	if (points >= 5 && !bossActive)
	{
		spawnBoss();
		std::cout << "Attempting to spawn boss." << std::endl;
		bossActive = true;
...
void GameScene::doCollisionLogic()
{
	...
	for (int i = 0; i < objects.size(); i++)
	{
		...
		if (boss)
		{
			std::cout << "Boss exists; Alive status: " << (boss->getIsBossAlive() ? "Alive" : "Dead") << std::endl;
		}
		else
		{
			std::cout << "Boss is nullptr" << std::endl; // This should only trigger if the boss is not in objects
		}
		...
		if (bullet != nullptr)
		{
			...
			{
				std::cout << "Player bullet detected" << std::endl;

				// Only proceed if the boss exists and is alive
				//if (boss != nullptr && boss->getIsBossAlive())
				//{
				//	std::cout << "Boss is not null and alive" << std::endl;
				...
				//	if (collision == 1)
				//	{
				//		std::cout << "Collision detected with boss!" << std::endl;
				//		boss->takeDamage(10);
				//		std::cout << "Boss health after damage: " << boss->getCurrentHealth() << "/" << boss->getMaxHealth() << std::endl;
				...
				//			points += 50;  // Add points for boss defeat

				//			// Update the score threshold for the next boss spawn
				//			nextBossSpawnScore = points + 100;

				//			bossActive = false;  // Mark the boss as defeated
				//			disableEnemySpawn = false;  // Allow normal enemies to spawn again
				//			std::cout << "Boss defeated! Next boss spawn at score: " << nextBossSpawnScore << std::endl;
				//		}	
				//	}
				//}
